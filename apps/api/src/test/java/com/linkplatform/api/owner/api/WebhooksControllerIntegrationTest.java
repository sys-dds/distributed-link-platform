...
@ActiveProfiles("test")
@DirtiesContext(classMode = DirtiesContext.ClassMode.BEFORE_EACH_TEST_METHOD)
class WebhookHostOnlyAllowanceIntegrationTest {
    ...
    @Test
    void privateHostAllowanceRelaxesHostChecksButStillRequiresHttps() throws Exception {
        String apiKey = bootstrapPersonalWorkspaceApiKey("host-only-key", "[\"webhooks:write\"]");

        mockMvc.perform(post("/api/v1/workspaces/current/webhooks")
                        .header("X-API-Key", apiKey)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"https-private","callbackUrl":"https://127.0.0.1:8443/callback","eventTypes":["link.created"],"enabled":true}
                                """))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.subscription.callbackUrl").value("https://127.0.0.1:8443/callback"));

        mockMvc.perform(post("/api/v1/workspaces/current/webhooks")
                        .header("X-API-Key", apiKey)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"http-public","callbackUrl":"http://hooks.example.com/callback","eventTypes":["link.created"],"enabled":true}
                                """))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.detail").value("Webhook callbackUrl must use https"));
    }
...
@ActiveProfiles("test")
@DirtiesContext(classMode = DirtiesContext.ClassMode.BEFORE_EACH_TEST_METHOD)
class WebhookVersionVisibilityIntegrationTest {
    ...
    void createdWebhookResponseExposesStableEventVersion() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/workspaces/current/webhooks")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"versioned","callbackUrl":"http://127.0.0.1:8080/hook","eventTypes":["link.created"],"enabled":false}
                                """))
                .andExpect(status().isCreated())
