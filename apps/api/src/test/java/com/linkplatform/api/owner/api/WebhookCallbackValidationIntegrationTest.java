...
@ActiveProfiles("test")
@DirtiesContext(classMode = DirtiesContext.ClassMode.BEFORE_EACH_TEST_METHOD)
class WebhookCallbackHttpOnlyAllowanceIntegrationTest {
    ...
    @Test
    void httpOnlyAllowanceRelaxesSchemeButStillRejectsPrivateHosts() throws Exception {
        String apiKey = bootstrapPersonalWorkspaceApiKey("http-only-key", "[\"webhooks:write\"]");
        ...
        mockMvc.perform(post("/api/v1/workspaces/current/webhooks")
                ...
                .andExpect(jsonPath("$.subscription.callbackUrl").value("http://hooks.example.com/callback"));

        mockMvc.perform(post("/api/v1/workspaces/current/webhooks")
                        .header("X-API-Key", apiKey)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"http-private","callbackUrl":"http://127.0.0.1:8080/callback","eventTypes":["link.created"],"enabled":true}
                                """))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.detail").value("Webhook callbackUrl must not target localhost or private addresses"));
    }
